...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!channel) return message.reply("Du musst dich in **einem Channel** befinden, bevor du den Befehl ausführen kannst.").catch(console.error);
    if (serverQueue && channel !== message.guild.me.voice.channel)
return message.reply(`Du musst in dem gleichen Channel wie **${message.client.user}** sein.`).catch(console.error);

    if (!args.length)
      return message
        .reply(`Benutze: **${message.client.prefix}play <YouTube URL>**`)
        .catch(console.error);
